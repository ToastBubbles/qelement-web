...
export default function Register() {
  ...
  if (questionData) {
    ...
    function GetUnusedQuestions(field: number): IPredefinedSecQuestionDTO[] {
      ...
      if (otherQA != -1) {
        const indexToRemove = output.findIndex((x) => x.questionId === otherQA);
        if (indexToRemove !== -1) {
          output.splice(indexToRemove, 1);
        }
      }
      if (otherQB != -1) {
        const indexToRemove = output.findIndex((x) => x.questionId === otherQB);
        if (indexToRemove !== -1) {
          output.splice(indexToRemove, 1);
        }
      }
        ...
        <div className="formcontainer">
          ...
          <div className="mainform">
            ...
            >
              ...
              {GetUnusedQuestions(1).map((questionList) => (
                <option
                  key={questionList.questionId}
                  value={`${questionList.questionId}
                >
            ...
            >
              ...
              {GetUnusedQuestions(2).map((questionList) => (
                <option
                  key={questionList.questionId}
                  value={`${questionList.questionId}`}
                >
            ...
            >
              ...
              {GetUnusedQuestions(3).map((questionList) => (
                <option
                  key={questionList.questionId}
                  value={`${questionList.questionId}`}
                >
